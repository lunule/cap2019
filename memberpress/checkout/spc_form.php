...
<div class="mp_wrapper">
  <form id="mepr-signup-form" class="mepr-signup-form mepr-form" method="post" action="<?php echo $_SERVER['REQUEST_URI'].'#mepr_jump'; ?>" novalidate>
    <input type="hidden" id="mepr_process_signup_form" name="mepr_process_signup_form" value="<?php echo isset($_GET['mepr_process_signup_form']) ? $_GET['mepr_process_signup_form'] : 1 ?>" />
    <input type="hidden" id="mepr_product_id" name="mepr_product_id" value="<?php echo $product->ID; ?>" />
    <input type="hidden" id="mepr_transaction_id" name="mepr_transaction_id" value="<?php echo isset($_GET['mepr_transaction_id']) ? $_GET['mepr_transaction_id'] : ""; ?>" />
    ...
    <?php if($product->adjusted_price($mepr_coupon_code) > 0.00 || !empty($product->plan_code)): ?>
      ...
      <div id="mepr-payment-methods-wrapper">
        <?php if ( 
                  ( sizeof( $payment_methods ) > 1 ) &&
                  ( !is_page_template( 'page-subscribe.php' ) )
                ) : ?>
